...
double distance(const segment2d& s1, const segment2d& s2) {
	if (intersect(s1, s2)) return 0.0;
	return std::min({ distance(s1.p1, s2), distance(s1.p2, s2), distance(s2.p1, s1), distance(s2.p2, s1) });
}
double area(const std::vector<point2d>& v) {
	double sum = 0.0;
...
int contain(const std::vector<point2d>& v, point2d p) {
...
std::vector<int> convex_hull(const std::vector<point2d>& v) {
	int N = v.size();
